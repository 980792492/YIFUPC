<template>
  <div id="review" class="wrap_review">
      <textarea ref="chatTextarea" class="review-content" :placeholder="placeholder" maxlength="150" v-model="content"></textarea>
...
//    上传本地图片按钮
      ...
      if (this.content) {
        Auth.isLogin().then(() => {
          ...
          const params = {
            ...
            reply_index: this.replyId,
          }
          e.target.disabled = true
          api.post('Room/SendComment', params).then(resp => {
            // console.log(resp)
            // 清空输入框
            this.content = ''
            this.replyId > -1 && this.handleCancelReply()
    ...
    // 取消评论
    handleCancelReply () {
      this.$store.commit(types.CANCEL_CHAT_REPLY)
  ...
  computed:{
    ...
    ...mapState({
      replyStatus: state => state.view.chat.status,
      replyId: state => state.view.chat.replyId,
      replyUserName: state => state.view.chat.replyUserName,
    }),
...
<style lang="less" scoped>
  ...
  .review-content { resize: none; width: 414px; height: 82px; overflow: hidden; border: none;outline: none; padding-top:10px; padding-left: 12px; padding-right: 12px;  display: block; font-size: 13px;}
  ...
  .reply-tag {max-width: 134px; overflow: hidden;}
</style>