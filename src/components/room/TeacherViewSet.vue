<template>
	<div class="viewset_wrap">
		<div class="nowtitle">
			<span class="font_red">*</span><span class="title">今日主题：</span><input class="title_text" type="text" v-model="title_text" maxlength="14" /><span class="text_number">限14个字</span><input class="btn" @click="save()"  type="button" value="保存" />
		</div>
		<div class="guest">
			<span class="title">邀请嘉宾：</span><input @input="inputlist" class="title_text" placeholder="此处仅显示最相似的10条数据" type="text" v-model="title_account" /><input @click="add" class="btn"  type="button" value="添加" />
...
//			设置可邀请嘉宾预查询列表
	...
	created(){
		this.api = new Api()
		this.title_text = this.roomData.TopicName
		// console.log(this.roomData)
		this.getInviteTeacher({ token: this.authToken })
	},
	methods:{
...
//		保存直播室今日主题
		save(){
			...
			params.topic_name = this.title_text
...
//		添加直播室嘉宾
		add(){
			...
			params.is_permanent = this.is_permanent ? 1 : 0
...
//		删除直播室嘉宾
	...
	computed: {
    ...mapGetters(['roomData','roomId', 'topicId', 'inviteTeacher', 'authToken'])
  }
}
</script>
